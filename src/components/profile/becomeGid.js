...
export default (inputHandler, inputListHandler, submitHandler, deleteHandle, fields, user, status) => {
    switch (status) {
        case STATUS_SUCCESS: return showSuccess('Ваша заявка принята. Ожидайте подтверждения модераторами')
        case STATUS_PROCESSING: return showLoading()

        default:
            return (
                <form action="POST" autoComplete="off" onSubmit={submitHandler}>
                    {user && !user.is_accepted ? <p>Ура! Ты почти стал гидом. В течение короткого времени мы проверим твой профиль и будет тебе счастье!</p> : null}
