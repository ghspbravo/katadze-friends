                    ...
                    <div className="col-md-4">
                        {
                            this.props.questionList.map((question, i) => <div className="col-12">
                                <button onClick={() => {this.setState({currentQuestion: i}); document.querySelector('#answer').scrollIntoView({behavior: 'smooth'})}} className="question"><p style={{color: i === this.state.currentQuestion ? "#41bfef" : 'black'}}>{question.title}</p></button>
                            </div>)
                        }
                    </div>
